...
**Описание:** Сброс всех локальных данных (избранное, удаленные, будущие черновики).
...
1. `resetFavorites()` - очищает избранное
2. `resetRemoved()` - очищает удаленные продукты
3. Инвалидирует RTK Query кеш для продуктов
